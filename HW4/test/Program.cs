...
public class HashFunction
{
    public static void Main()
    {
        ...
        while (q<NOQ)
        {
            ...
            for (int j=ToDo.Length-1 ; j>=0 ; j--)
            {
                SumOfFirstMod = (SumOfFirstMod * 263 + ToDo[j]) % 1000000007;

            }
            SumOfFirstMod = SumOfFirstMod%size;
            ...
            {
                if (ArrayOfList[SumOfFirstMod].Contains(ToDo)==false)
                ArrayOfList[SumOfFirstMod].Insert(0,ToDo);
            }
            else if (input.StartsWith("del"))
            {
                try
                {ArrayOfList[SumOfFirstMod].Remove(ToDo);}
                catch
                {continue;}
            }
            ...
            {
                if (ArrayOfList[SumOfFirstMod].Contains(ToDo))
                {
